...
func (k *hermesKeyer) KeyOn() error {
	if k.conn == nil {
		return fmt.Errorf("hermes: keyer is closed")
	}
	if !C.hermes_ptt_on(k.conn) {
		return fmt.Errorf("hermes: PTT ON: no response from radio controller")
	}
	return nil
}
...
func (k *hermesKeyer) KeyOff() error {
	...
	if !C.hermes_ptt_off(k.conn) {
		return fmt.Errorf("hermes: PTT OFF: no response from radio controller")
	}
	return nil
}
